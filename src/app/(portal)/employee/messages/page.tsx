...
export default async function EmployeeMessagesPage({ searchParams }: { searchParams: Promise<{ contactId?: string }> }) {
  const user = await getCurrentUser();
  if (!user || user.role !== 'EMPLOYEE') redirect('/auth/login');

  const { contactId } = await searchParams;

  // Employees can only message Admins
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
      <div className="mb-4">
        ...
        <p className="text-[13px] text-[var(--color-slate)] font-switzer">Communicate with administrators.</p>
      </div>
        ...
        <div className="w-[300px] border-r border-[var(--color-silver)] flex flex-col h-full bg-[var(--color-bone)]">
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {contacts.map((contact) => (
              <Link key={contact.id} href={`/employee/messages?contactId=${contact.id}`}>
                <div className={`p-3 rounded-[12px] cursor-pointer transition-colors flex items-center gap-3 ${activeContact?.id === contact.id ? 'bg-[var(--color-paper)] border border-[var(--color-silver)] shadow-sm' : 'hover:bg-[var(--color-paper)]'}`}>
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="flex justify-between items-center mb-1">
                      <p className="text-[14px] font-semibold text-[var(--color-obsidian)] font-switzer truncate">{contact.name || 'Admin'}</p>
                    </div>
            ...
            <form action={async (formData) => {
              ...
              await prisma.notification.create({
                data: {
                  ...
                  title: 'New Message',
                  body: `You received a new message from ${user.name || 'Employee'}`,
                  type: 'MESSAGE',
                  link: activeContact.role === 'ADMIN' ? '/admin/messages' : '/messages'
                }
              });
