...
export default async function ClientMessagesPage({ searchParams }: { searchParams: Promise<{ contactId?: string }> }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
      <div className="mb-4">
        ...
        <p className="text-[13px] text-[var(--color-slate)] font-switzer">Communicate directly with MMTPL administrators.</p>
      </div>
        ...
        <div className="w-[300px] border-r border-[var(--color-silver)] flex flex-col h-full bg-[var(--color-bone)]">
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {contacts.map((contact) => (
              <Link key={contact.id} href={`/client/messages?contactId=${contact.id}`}>
            ...
            <form action={async (formData) => {
              ...
              await prisma.notification.create({
                data: {
                  ...
                  title: 'New Message',
                  body: `You received a new message from ${user.name || 'Client'}`,
                  type: 'MESSAGE',
