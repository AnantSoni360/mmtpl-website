...
export default function CareersClient({ jobs, isLoggedIn }: CareersClientProps) {
  ...
  const handleApplyClick = (jobTitle: string, jobId: string) => {
    if (!isLoggedIn) {
      // Redirect to register/login or show modal
            ...
            <div className="relative z-10 text-center mb-10">
              ...
              <p className="body-text text-slate">
                {selectedJob 
                  ...
                  : 'Please select a job above to apply.'}
              </p>
            </div>
            
            {!isLoggedIn ? (
              <div className="p-8 bg-bone border border-silver rounded-2xl font-switzer text-center relative z-10">
                <h3 className="font-bold text-xl mb-3 text-obsidian">Login Required</h3>
                <p className="mb-6 text-slate">You must create an account to apply for positions and track your application status.</p>
                <Button onClick={() => router.push('/auth/register')} variant="lilac">Register / Log In</Button>
              </div>
