...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (data.role === 'CLIENT') router.push('/client');
      else if (data.role === 'JOB_SEEKER') router.push('/jobs');
      else router.push('/');
    } catch (err: any) {
    ...
    <div className="relative min-h-screen w-full overflow-hidden flex bg-[#f8f9fa] dark:bg-[#050B1A] transition-colors duration-500">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          ...
          priority
        />
        {/* Navy/Blue-Purple Gradient Overlay for Dark, Light Gradient for Light Mode */}
      ...
      <div className="relative z-10 flex w-full h-screen">
        ...
        <div className="w-full lg:w-[40%] flex justify-center items-center px-6 lg:px-12 relative">
          <div className={`w-full max-w-[420px] transition-all duration-1000 delay-500 transform ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'}`}>
            ...
            <div className="relative rounded-2xl bg-white/80 dark:bg-[#081122]/40 backdrop-blur-2xl border border-gray-200 dark:border-white/10 shadow-xl dark:shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden group transition-colors duration-500">
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-8">
                  ...
                  <h2 className="text-2xl font-semibold font-clash text-[#111] dark:text-white mb-2 transition-colors duration-500">Welcome Back</h2>
                  <p className="text-[14px] text-gray-500 dark:text-[#B0B8D0] font-switzer transition-colors duration-500">Sign in to your enterprise portal</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="relative group/input">
                    <div className="flex justify-between items-end mb-1.5">
                      <label className="block text-[12px] font-medium text-gray-600 dark:text-[#B0B8D0] font-switzer uppercase tracking-wider transition-colors duration-500">
                        Password
                      </label>
                      <a href="#" className="text-[12px] font-switzer text-[#2563EB] dark:text-[#00E5FF] hover:text-[#1D4ED8] dark:hover:text-white transition-colors">Forgot Password?</a>
                    </div>
                    <input
                      type="password"
                      value={password}
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <input type="checkbox" id="remember" className="rounded border-gray-300 dark:border-white/20 bg-white dark:bg-white/5 text-[#2563EB] focus:ring-[#2563EB]" />
                    <label htmlFor="remember" className="text-[13px] font-switzer text-gray-600 dark:text-[#B0B8D0] transition-colors duration-500">Remember Me</label>
                  </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 dark:border-white/10 text-center transition-colors duration-500">
                  <p className="text-[13px] font-switzer text-gray-600 dark:text-[#B0B8D0] transition-colors duration-500">
                    New User?{' '}
                    <Link href="/auth/register" className="text-[#2563EB] dark:text-[#00E5FF] hover:text-[#1D4ED8] dark:hover:text-white font-medium transition-colors">
                      Create Account
                    </Link>
                  </p>
                </div>
